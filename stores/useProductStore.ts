  ...
  immer((set, get) => ({
    ...
    fetchProducts: async (customerId) => {
      ...
      try {
        ...
        set((state) => { state.products = products; state.activeProductId = null; });
      } finally {
        set((state) => { state.isLoading = false; });
      }
    },
    setSearch: (value) => set((state) => { state.search = value; }),
    setCategory: (value) => set((state) => { state.category = value; }),
    setActiveProduct: (id) => set((state) => { state.activeProductId = id; }),
    addProduct: async (product, customerId) => {
      const response = await fetch("/api/products", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...product, customerId }),
      });
      if (!response.ok) throw new Error("Failed to add product");
      const saved = (await response.json()) as Product;
      set((state) => { state.products.unshift(saved); });
    },
    ...
    replaceProducts: (products) => set((state) => {
      const existingById = new Map(state.products.map((p) => [p.id, p]));
      state.products = products.map((p) => ({ ...p, ...(existingById.get(p.id) ?? {}) }));
    ...
    removeProduct: async (id) => {
      ...
      set((state) => {
        ...
        if (state.activeProductId === id) state.activeProductId = null;
      });
    },
    ...
    upsertProduct: (product) => set((state) => {
      ...
      if (idx === -1) state.products.push(product);
      else state.products[idx] = { ...state.products[idx], ...product };
    }),
  })),
);
