...
export const usePlacementStore = create<PlacementStore>()(
  immer((set, get) => ({
    ...
    past: [],
    future: [],
    selectPlacement: (id) => set((state) => { state.selectedPlacementId = id; }),
    placeProduct: ({ pallet, product, wall, shelfRow, gridCol }) => {
      ...
      const conflict = detectPlacementConflict(get().placements, {
        wall, shelfRow, gridCol: clampedCol, colSpan,
      });
      if (conflict) return { ok: false, reason: "Cell is already occupied." };
      ...
      set((state) => {
        ...
        state.placements.push({
          id, productId: product.id, wall, shelfRow, gridCol: clampedCol,
          colSpan, rotation: 0, quantity: 1, displayMode: "face-out",
        });
    ...
    removeSelected: () => set((state) => {
      ...
      state.past.push(clonePlacements(state.placements));
      state.future = [];
      state.placements = state.placements.filter((p) => p.id !== state.selectedPlacementId);
    ...
    updatePlacement: (id, patch) => set((state) => {
      const placement = state.placements.find((e) => e.id === id);
      if (!placement) return;
      state.past.push(clonePlacements(state.placements));
      state.future = [];
    ...
    rotateSelected: (products, pallet) => {
      ...
      const placement = get().placements.find((e) => e.id === selectedPlacementId);
      if (!placement) return;
      const product = products.find((e) => e.id === placement.productId);
      if (!product) return;
      ...
      const conflict = detectPlacementConflict(
        get().placements,
        { wall: placement.wall, shelfRow: placement.shelfRow, gridCol: nextCol, colSpan: nextColSpan },
        placement.id,
      );
      if (conflict) return;
      set((state) => {
        state.past.push(clonePlacements(state.placements));
        state.future = [];
        const selected = state.placements.find((e) => e.id === selectedPlacementId);
        if (!selected) return;
    ...
    replacePlacements: (placements) => set((state) => {
