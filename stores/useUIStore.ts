...
export const useUIStore = create<UIStore>()(
  persist(
    immer((set) => ({
      ...
      dropPreview: null,
      setSelectedCustomerId: (id) => set((state) => { state.selectedCustomerId = id; }),
      setSelectedWall: (face) => set((state) => { state.selectedWall = face; }),
      setViewMode: (mode) => set((state) => { state.viewMode = mode; }),
      setCameraView: (view) => set((state) => { state.cameraView = view; }),
      setDraggingProductId: (productId) => set((state) => { state.draggingProductId = productId; }),
      setDropPreview: (preview) => set((state) => { state.dropPreview = preview; }),
    })),
    {
      name: "kayco-ui-store",
